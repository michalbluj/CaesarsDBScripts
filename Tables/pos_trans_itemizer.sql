CREATE TABLE caesars.pos_trans_itemizer (
  id BIGSERIAL,
  c_prop_cd VARCHAR(3),
  c_pos_trans_id VARCHAR(50),
  i_pos_trans_itemizer_seq_id INTEGER,
  ...
  c_itemizer_type_desc TEXT,
  f_itemizer_amt NUMERIC(10,2),
  c_quality_cd VARCHAR(12),
  d_timestamp TIMESTAMP WITHOUT TIME ZONE,
  c_file_prop_cd VARCHAR(3),
  CONSTRAINT pos_trans_itemizer_pkey PRIMARY KEY(id)
) 
WITH (oids = false);